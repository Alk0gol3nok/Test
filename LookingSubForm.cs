...
{
    public partial class LookingSubForm : Form
    {
        ...
        private void looking_button_Click(object sender, EventArgs e)
        {
            ...
            String metka_checker = role_change_box.Text;

            DataBase db = new DataBase();
            DataTable table = new DataTable();

            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("SELECT * FROM `users` WHERE `metka` = @mtk", db.getConnection());

            command.Parameters.Add("@mtk", MySqlDbType.VarChar).Value = metka_checker;
